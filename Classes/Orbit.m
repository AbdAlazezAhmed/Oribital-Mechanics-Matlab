classdef Orbit
    %ORBIT 
    %All properties are public for debugging purposes
    properties
        t
        mu = 398600;
        R_e = 6378;
    ...
    methods
            ...
            switch obj.method
                ...
                case 'Cowell'
                    options = odeset('reltol', 1.e-6, ...
                        ...
                        'events', @terminate);
                    if obj.drag == true
                        [ts,ys] = ode45(@(t,y)gravity(y,obj.mu,drag = obj.drag,C_d = 2.2,omega_e = obj.omega_earth,R_e = obj.R_e, A = 1/4, m = 100),linspace(t_1,t_f,obj.res),[obj.R_0;obj.V_0],options);
                    else
        ...
        function obj = gibbs(obj)
            ...
            obj = orbitaElementsFromVectors(obj,obj.R2,V2);
        end
    end
end
